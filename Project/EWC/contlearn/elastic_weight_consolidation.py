...
class ElasticWeightConsolidation:
    ...
    def _update_fisher_params(self, current_ds, batch_size, num_batch):
        ...
        for i, (input, target) in enumerate(dl):
            if i > num_batch:
                break
            target = target.squeeze(dim=1)  # 수정
            output = F.log_softmax(self.model(input), dim=1)
            log_liklihoods.append(output[:, target.long()])  # 수정 target.long()
    ...
    def forward_backward_update(self, input, target):
        ...
        target = target.squeeze(dim=1)  # 수정
        loss = self._compute_consolidation_loss(self.weight) + self.crit(output, target.long())  # 수정: target.long()
