...
'''
captum 모듈: 특징(features)이 모델의 예측 또는 뉴런 활성화에 미치는 영향을 이해하고, 모델의 동작 방식을 알 수 있음.
...
# wandb 사용을 위함
wandb.init(project="mnist", entity="wonjeong")
...
'''
    find_lr: 모델을 학습하지 않고 learning rate 바꿔가며 loss 관찰하는 함수.
    데이터셋에 대해 lr을 바꿔가며 첫 10번 epoch의 train loss 판단.
    보통 loss 최고점~최저점의 1/3 지점이 최적의 loss라고 함 (경험적으로)
    
    warm-up learning: (확실하지 않아 다시 찾아봐야됨!!) 약 10 epoch 돌려보며 초기 weight 판단하는 개념..?
'''
